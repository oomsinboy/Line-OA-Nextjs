...
const InfoBody = ({ items, onFetchInfo }: InfoBodyProps) => {
    ...
    // const quillRef = useRef<any>(null); // ใช้ useRef สำหรับ ReactQuill instance
    ...
    const handleDelete = async (e: React.MouseEvent<HTMLButtonElement>, id: number) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                await axios.delete(`${process.env.NEXT_PUBLIC_CALLAPI}information/task/${id}`);
    ...
    const handleViewClick = async (id: number) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_CALLAPI}information/task/${id}`);
    ...
    const handleEditClick = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_CALLAPI}information/task/${modalData.id}`, formdata);

            if (response.status === 200) {
                // setIsEditing(false);
                Swal.fire({
                    title: 'สำเร็จ',
                    text: 'คุณได้ทำการแก้ไขบทความเรียบร้อยแล้ว',
                    icon: 'success',
                    confirmButtonText: 'ปิด',
                }).then(() => {
                    onFetchInfo();
    ...
    const handleSaveInfo = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_CALLAPI}information`, formdata);

            if (response.status === 200) {
                // setIsEditing(false);
                Swal.fire({
                    title: 'สำเร็จ',
                    text: 'คุณได้ทำการเพิ่มบทความเรียบร้อยแล้ว',
                    icon: 'success',
                    confirmButtonText: 'ปิด',
                }).then(() => {
                    onFetchInfo();
            ...
            <div className='h-full w-full rounded-[15px] bg-white p-5 '>
                <div className='min-h-[81.71dvh] flex flex-col'>
                    <div className='flex justify-between'>
                        <div>
                            <div className='text-xl 2xl:text-2xl text-[#5955B3] font-semibold'>บทความ</div>
                    ...
                    <div className='pt-4 flex-grow'>
                        <div className='flex flex-wrap -mx-2'>
                            {currentItems.map((item) => (
                                <ItemCard key={item.id} item={item} onDelete={handleDelete} onView={handleViewClick} title="info" />
                            ))}
                        </div>
                    </div>
                </div>
                <Modal isVisible={showModal} onClose={handleCloseModal} title={textMode}>
                    <div className='p-5'>
                        ...
                        <div className='mt-2'>
                            ...
                            {!(modalData && [2, 3, 4, 5, 6, 7, 8, 9].includes(modalData.id)) && (
